...
// utils is a library of generic helper functions non-specific to ipoa
...
/**
 * Determine if a value is an JSON
 *
 * @param str
 * @param passObject
 * @returns {boolean} True if value is an JSON, otherwise false
 */
function isJSON(str, passObject = false) {
  if (isObject(str) && passObject) return true
...
function getUrlParams(url = '') {
  const keyValueArr = url && url.split('?')[1].split('&')
  var paramObj = {}
  forEach(keyValueArr, function (item) {
...
module.exports = {
  ...
  isProcessEnv: isProcessEnv,
  isProcessFormData: isProcessFormData,
  forEach: forEach,
  merge: merge,
  extend: extend,
  trim: trim,
